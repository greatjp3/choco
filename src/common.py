...
try:
    ...
    logger.error(f"gTTS import exception\n")
...
def speak_ack():
    ...
    try:
        ...
        logger.error("음성을 재생할 수 없습니다.")
    except Exception as e:
        logger.write(f"⛔ 음성 출력 중 오류 발생: {e}\n")
        logger.error("음성 출력 중 오류가 발생했습니다.")
...
def stop_speech():
    ...
    with tts_lock:
        if tts_play_obj and is_speeching:
            tts_play_obj.stop()
            is_speeching = False
            tts_play_obj = None
            logger.info("🔇 TTS 중단됨")
        else:
            logger.info("🔈 현재 재생 중인 TTS 없음")
